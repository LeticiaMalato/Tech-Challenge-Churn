...
class CustomerFeatures(BaseModel):
    ...
    # Contrato
    tenure_months:     int   = Field(..., ge=0,  examples=[24],         description="Meses de contrato (≥ 0)")
    contract:          str   = Field(..., examples=["Month-to-month"])
    paperless_billing: str   = Field(..., examples=["Yes"],             description="Yes ou No")
    payment_method:    str   = Field(..., examples=["Electronic check"])

    # Financeiro
    monthly_charges:   float = Field(..., gt=0,  examples=[65.50],      description="Cobrança mensal (> 0)")
    ...
    @field_validator("senior_citizen", "partner", "dependents",
    ...
    def validate_yes_no(cls, v: str) -> str:
        ...
        return v
    @field_validator("multiple_lines", "online_security", "online_backup",
...
class PredictionResponse(BaseModel):
    ...
    probability:   float         = Field(..., ge=0.0, le=1.0, description="Probabilidade de churn")
