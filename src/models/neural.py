...
def create_tensors(X_train_sc, X_val_sc, X_test_sc, y_train, y_val, y_test):
    ...
    to_tensor = lambda arr: torch.tensor(np.array(arr).astype(float), dtype=torch.float)
...
def mlp(X_train_sc, X_val_sc, X_test_sc,
                y_train, y_val, y_test, dataset_meta: dict):
    ...
    with mlflow.start_run(run_name="mlp_pytorch"):
        ...
        for epoch in range(EPOCHS):
            ...
            print(f"Época {epoch+1:>5} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f}")
        ...
        for t in [0.3, 0.4, 0.5]:
            ...
            print(f"  Threshold {t} → F1: {f1_score(y_test, pred_t):.4f} | "
