...
def plot_target_distribution(df: pd.DataFrame):
    ...
    if ratio > 3:
        print(f"⚠️  Dataset desbalanceado ({ratio:.1f}:1) → usar PR-AUC e Recall como métricas principais.")
...
def plot_histograms(df: pd.DataFrame, num_cols: list):
    ...
    for ax, col in zip(axes.ravel(), num_cols):
        ...
        ax.axvline(media,   color="#E24B4A", linestyle="--", linewidth=1.5, label=f"Média={media:.1f}")
        ax.axvline(mediana, color="#F39C12", linestyle=":",  linewidth=1.5, label=f"Mediana={mediana:.1f}")
...
def plot_bivariate(df: pd.DataFrame, num_cols: list):
    ...
    for ax, col in zip(axes.ravel(), num_cols):
        ...
        bp = ax.boxplot([g0, g1], patch_artist=True, labels=["Não Churn", "Churn"],
    ...
    plt.suptitle("Distribuição por Grupo (Churn vs. Não Churn)\nTeste Mann-Whitney",
                 fontsize=13, y=1.01)
    plt.tight_layout()
    plt.show()
